...
for kk = 1:N.file
    ...
    SACCADE{kk,5} = {struct(head_saccade)}; % store data in cell
    ...
    if any(abs(head_saccade.SACD.PeakVel) < 300)
...
%         wing.right      = interp1(pat.time_sync, wing.right, tintrp, 'pchip');
        ...
        wing.dwba_vel   = central_diff(wing.dwba, 1/Fs); % velocity
...
%             pause
        ...
        SACCADE{kk,6} = {struct(wing_saccade)};
        ...
        if head_saccade.count > 0
            ...
            %pause
        	SACCADE{kk,7} = {struct(head2wing)};
            SACCADE{kk,8} = {struct(head2wing_all)};
        end
    end
end
