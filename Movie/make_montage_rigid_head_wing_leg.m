function [MOV] = make_montage_rigid_head_wing_leg(rootdir,rootpat,vidFs,export)
...
rootleg = 'Q:\Google Drive PSU\Experiment_Data';
pat_ypos = 5;
...
dlc_files       = dir(PATH.dlc);
dlc_files    	= string({dlc_files.name}');
FILE.dlc        = start_end_string(dlc_files, FILE.basename, '.csv');
...
dlc_data        = readDLC(fullfile(PATH.dlc,FILE.dlc)); % load DLC data
disp('DONE')
...
FLY.int_leg_left  = FLY.leg_left(TRIG.range,:);
FLY.int_leg_right = FLY.leg_right(TRIG.range,:);
...
for jj = 1:FLY.nframe % for each frame
    if expframe(jj) % if we want to display this frame
        ...
        subplot(2,4,[1:2,5:6]); cla; hold on; axis image
            ...
            plot(mean(FLY.int_leg_left(win,1)), mean(FLY.int_leg_left(win,2)), 'm.', 'MarkerSize',20)
