function [MOV] = make_montage_rigid_head_wing_pat_flyview(rootdir,rootpat,vidFs,export)
%% make_montage_rigid_head_wing_pat_flyview: makes movie for fly in rigid tether
%
...
FILE.montage    = [FILE.basename '_Montage_FlyView.mp4'];
...
FLY.error = 3.75*PAT.pos_exp - FLY.head;
...
%PAT.int_norm_filt = 3.75*(PAT.int_pos_filt - PAT.int_pos_filt(1));
...
clear ax
ax(1) = subplot(3,4,[1:2,5:6]) ; cla; hold on; axis square % for raw fly & pattern vid
ax(2) = subplot(3,4,[3:4]) ; cla; hold on
...
ax(3) = subplot(3,4,[7:8]) ; cla; hold on
...
ax(4) = subplot(3,4,[9:10]); cla ; axis image; axis off
set(ax(4), 'YColor', 'none', 'XColor', 'none')
...
for jj = 1:FLY.nframe % for each frame
    if expframe(jj) % if we want to display this frame
        ...
        % Display raw video
        subplot(3,4,[1:2,5:6]); cla; hold on; axis image
            imshow(Frame.raw)
            ...
            panel_shift = round(mean(PAT.pos_exp(win)));
            error_shift = round(mean(FLY.error(win)));
            
            pat_pos = 3.75*panel_shift;
            %error_pos = 3.75*error_shift;
        ...
        subplot(3,4,[9:10]); cla ; axis image; axis tight
            Y1 = circshift(pat_image, panel_shift,2);
            Y2 = circshift(pat_image, error_shift,2);
            imagesc([Y1;Y2])
          	axis off
