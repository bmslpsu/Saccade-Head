...
movegui(FIG,'center')
FIG.Color = 'w';
ax = gobjects((N.vel/2),2);
clear h
for jj = 1:N.vel/2
    ax(jj,1) = subplot(2,clms,jj) ; hold on
    title([num2str(Vel(jj)) ' (°/s)']);
        time = IntSort.normstart_interval(jj).time;
        pos = IntSort.norm_interval(jj).position;
    ...
    ax(jj,2) = subplot(2,clms,jj+N.vel/2) ; hold on
    title([num2str(Vel(jj+clms)) ' (°/s)']);
        time = IntSort.normstart_interval(jj + N.vel/2).time;
        pos = IntSort.norm_interval(jj + N.vel/2).position;
    	med_time = nanmedian(time,2);
        med_pos = nanmedian(pos,2);
        std_pos = nanstd(pos,[],2);
...
for jj = 1:N.vel/2
    ...
    title([num2str(Vel(jj)) ' (°/s)']);
        time = IntSort.normstart_interval(jj).time;
        pos = IntSort.norm_interval(jj).velocity;
        med_time = nanmedian(time,2);
        ...
        plot([0 2],0*[1 1],'-','Color','k','LineWidth',1)
         plot([0 2], Vel(jj)*[1 1], '--', 'Color', 'k', 'LineWidth', 1)
        
    ax(jj,2) = subplot(2,clms,jj+N.vel/2) ; hold on
    title([num2str(Vel(jj+clms)) ' (°/s)']);
        time = IntSort.normstart_interval(jj + N.vel/2).time;
        pos = IntSort.norm_interval(jj + N.vel/2).velocity;
    	med_time = nanmedian(time,2);
        med_vel = nanmedian(pos,2);
        std_vel = nanstd(pos,[],2);
