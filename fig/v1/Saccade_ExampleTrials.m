...
[b, a] = butter(3, 40 / (trial.Fs/2), 'low');
filt_pos = filtfilt(b, a, trial.position);
stim_pos = trial.stimlus_position;
trial = saccade_all(filt_pos, trial.time, trial.threshold, trial.true_thresh, trial.Fc_detect, ...
                            trial.Fc_ss, trial.amp_cut, trial.direction, [], nan, ...
                            trial.min_pkdist, trial.min_pkwidth, trial.min_pkprom, ...
                            trial.min_pkthresh, trial.boundThresh, false);
...
ax(4) = subplot(6,4,7:8) ; hold on
    h.main(4) = plot(trial.time, trial.velocity_filt_detect, 'Color', 'k');
 	plot([0 10],  [trial.threshold trial.threshold], 'Color', 'm');
  	%h.main(1) = plot([0 10], -[trial.threshold trial.threshold], 'Color', 'm');
