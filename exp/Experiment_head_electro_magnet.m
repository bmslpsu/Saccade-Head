function [] = Experiment_head_electro_magnet(Fn, ctrl)
%% Experiment_head_electro_magnet: runs a experiment using the LED arena and fly panel
% Fn is the fly number
daqreset
imaqreset
% Fn = 0;
%% Set directories & experimental parameters
root = 'C:\BC\Rigid_data\Experiment_head_electro_magnet';
...
switch ctrl
    ...
    case 1
        id = 1;
    otherwise
        error('ctrl must be equal to 0 or 1')
end
...
seq = [0*ones(off_samp,1) ; ctrl*5*ones(on_samp,1)]; % one on-off sequence
...
for ii = 1:n_rep
    ...
    queueOutputData(s, AO_all) % set AO signals
    tic
    ...
    disp('Saving...')
    disp('-----------------------------------------------------------------')
    fname = ['fly_' num2str(Fn) '_trial_' num2str(ii) '_control_' num2str(id) '.mat'];
